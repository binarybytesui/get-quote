...
<head>
  ...
  <style>
    ...
    td.value{min-width:70px}
    tr:hover{background:#fbfcfd}
    .action-icon{cursor:pointer;padding:6px;border-radius:6px;margin-left:6px;font-size:14px}
    .action-icon:hover{background:#f3faf9}
    .action-icon.delete:hover{background:#fff0f0}
    .tabs{display:flex;gap:8px;margin-bottom:12px}
    .tab{padding:8px 12px;border-radius:8px;cursor:pointer;background:transparent;border:1px solid #eef2f7}
    .tab.active{background:#fff;border-color:#e6fffb;box-shadow:0 1px 2px rgba(0,0,0,.03)}
  ...
  for(const p of list){
    ...
    tr.innerHTML = `
      ...
      <td class="small actions-col">
        <span class="action-icon edit" data-action="edit" title="Edit">✏️</span>
        <span class="action-icon delete" data-action="delete" title="Trash">🗑️</span>
      </td>
    `;
...
function renderTrashTable(){
  ...
  for(const p of trash){
    ...
    tr.innerHTML = `
      ...
      <td class="small">${formatNumber(p.price)}</td>
      <td class="small actions-col">
        <span class="action-icon restore" data-action="restore" title="Restore">♻️</span>
        <span class="action-icon delete" data-action="delete" title="Delete Permanently">🗑️</span>
      </td>
    `;
